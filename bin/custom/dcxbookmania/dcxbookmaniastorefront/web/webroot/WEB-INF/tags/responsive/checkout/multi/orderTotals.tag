...
<%@ attribute name="subtotalsCssClasses" required="false"
...
<div class="subtotals ${fn:escapeXml(subtotalsCssClasses)}">
	<div class="subtotal" style="padding: 0 0 0 12px;">
		<spring:theme code="basket.page.totals.subtotal" />
		<span> <ycommerce:testId code="Order_Totals_Subtotal">
	...
	<c:if test="${cartData.totalDiscounts.value > 0}">
		<div class="subtotals__item--state-discount">
			...
			<span> <ycommerce:testId code="Order_Totals_Savings">
	...
	<c:if test="${cartData.quoteDiscounts.value > 0}">
		<div class="subtotals__item--state-discount">
			...
			<span> <ycommerce:testId code="Quote_Totals_Savings">
	...
	<c:if test="${not empty cartData.deliveryCost}">
		<div class="shipping">
			...
			<span> <ycommerce:testId code="Order_Totals_Delivery">
	...
	<c:if test="${cartData.net && cartData.totalTax.value > 0 && showTax}">
		<div class="tax">
			...
			<span> <format:price priceData="${cartData.totalTax}" />
			</span>
		</div>
	</c:if>
	<div class="totals"style="padding: 0 0 0 12px;">
		<spring:theme code="basket.page.totals.total" />
		<span> <ycommerce:testId code="cart_totalPrice_label">
				<c:choose>
					<c:when test="${showTax}">
	...
	<c:if test="${not cartData.net}">
		<div class="realTotals" style="padding: 0 0 0 15px;">
