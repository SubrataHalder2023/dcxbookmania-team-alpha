		...
		<c:forEach items="${checkoutSteps}" var="checkoutStep"
			...
			<c:choose>
				<c:when test="${progressBarId eq checkoutStep.progressBarId}">
					...
					<a href="${fn:escapeXml(stepUrl)}"
						class="step-head js-checkout-step active"
						style="background-color: #FBB917; border-radius: 25px;display: inline-block;">
					...
					<div class="step-body"
						style="background-color: #89C35C; border-radius: 25px;-webkit-text-stroke-width: medium;display: flex"><jsp:doBody /></div>
				</c:when>
				<c:when test="${checkoutStep.stepNumber > activeCheckoutStepNumber}">
					<a href="${fn:escapeXml(stepUrl)}"
						class="step-head js-checkout-step "
						style="background-color: #FBB917; border-radius: 25px;display: inline-table;">
						<div class="title">
							<spring:theme code="checkout.multi.${checkoutStep.progressBarId}" />
						</div>
					</a>
				...
				<c:otherwise>
					<a href="${fn:escapeXml(stepUrl)}"
						class="step-head js-checkout-step" style="background-color: #FBB917; border-radius: 25px;">
						<div class="title">
							<spring:theme code="checkout.multi.${checkoutStep.progressBarId}" />
						</div>
						<!-- div class="edit">
							<span class="glyphicon glyphicon-pencil" style="background-color: red"></span>
						</div-->
					</a>
				</c:otherwise>
			</c:choose>
