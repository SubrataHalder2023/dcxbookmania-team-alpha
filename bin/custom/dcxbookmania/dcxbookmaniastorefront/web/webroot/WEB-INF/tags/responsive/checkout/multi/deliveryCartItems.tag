...
<ul class="checkout-order-summary-list">
	<c:if test="${hasShippedItems}">
		<li class="checkout-order-summary-list-heading" 
			style="border-radius: 25px; background-color: #FBB917 !important; padding: 15px 0px 15px 10px; font-weight: bold; color: white;width: 62rem;font-size:18px">
			<c:choose>
		...
		<c:if test="${entry.deliveryPointOfService == null}">
			...
			<li class="checkout-order-summary-list-items">
				...
				<div class="details">
					...
					<div>
						<span class="label-spacing"><spring:theme
								code="order.itemPrice" />:</span>
						<c:if test="${entry.product.multidimensional}">
							<%-- if product is multidimensional with different prices, show range, else, show unique price --%>
							<c:choose>
								<c:when
									test="${entry.product.priceRange.minPrice.value ne entry.product.priceRange.maxPrice.value}">
									<format:price priceData="${entry.product.priceRange.minPrice}" /> - <format:price
										priceData="${entry.product.priceRange.maxPrice}" />
								</c:when>
								<c:when
									test="${entry.product.priceRange.minPrice.value eq entry.product.priceRange.maxPrice.value}">
					...
					<div class="qty">
						<span><spring:theme code="basket.page.qty" />:</span>${fn:escapeXml(entry.quantity)}</div>
					<div>
						<c:forEach items="${entry.product.baseOptions}" var="option">
							<c:if
								...
								<c:forEach items="${option.selected.variantOptionQualifiers}"
									var="selectedOption">
									<div>${fn:escapeXml(selectedOption.name)}:
										${fn:escapeXml(selectedOption.value)}</div>
								</c:forEach>
							</c:if>
						</c:forEach>
					...
					<spring:url value="/checkout/multi/getReadOnlyProductVariantMatrix"
						var="targetUrl" htmlEscape="false" />
					<grid:gridWrapper entry="${entry}" index="${loop.index}"
						styleClass="display-none" targetUrl="${targetUrl}" />
				</div>
			</li>
		</c:if>
	</c:forEach>
