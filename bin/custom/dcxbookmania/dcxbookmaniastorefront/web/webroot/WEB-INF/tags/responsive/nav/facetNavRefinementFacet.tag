	...
	<ycommerce:testId code="facetNav_title_${facetData.name}">
		<div class="facet js-facet">
			<div class="facet__name js-facet-name"
				style="background-color: #FBB917; border-radius: 26px !important; padding: 10px 10px 10px 10px; font-family: 'Dosis'; font-size: 19px; margin: 0px 102px 0px 0px;">
			...
			<div class="facet__values js-facet-values js-facet-form">
							...
							<li><c:if test="${facetData.multiSelect}">
									<form action="#" method="get">
										...
										<input type="hidden" name="q"
											...
											value="${searchPageData.freeTextSearch}" /> <label> <input
											class="facet__list__checkbox" type="checkbox"
				...
				<ul
					class="facet__list js-facet-list <c:if test="${not empty facetData.topValues}">facet__list--hidden js-facet-list-hidden</c:if>">
					<c:forEach items="${facetData.values}" var="facetValue">
						<li><c:if test="${facetData.multiSelect}">
								<ycommerce:testId code="facetNav_selectForm">
									<form action="#" method="get">
										...
										<input type="hidden" name="q"
											...
											type="checkbox"
											${facetValue.selected ? 'checked="checked"' : ''}
											class="facet__list__checkbox js-facet-checkbox sr-only" /> <span
											class="facet__list__label"> <span
												class="facet__list__mark"></span> <span
												class="facet__list__text"
												style="background-color: #89C35C; border-radius: 26px !important; padding: 5px 5px 5px 5px; font-family: 'Dosis'; font-size: 15px; margin: 0px 100px 0px 0px;">
													${fn:escapeXml(facetValue.name)}&nbsp; <ycommerce:testId
														code="facetNav_count">
														<span class="facet__value__count"><spring:theme
																code="search.nav.facetValueCount"
								...
								<c:url value="${facetValue.query.url}" var="facetValueQueryUrl" />
								<span class="facet__text"> <a
									href="${fn:escapeXml(facetValueQueryUrl)}">${fn:escapeXml(facetValue.name)}</a>
									<ycommerce:testId code="facetNav_count">
										<span class="facet__value__count"><spring:theme
												code="search.nav.facetValueCount"
												arguments="${facetValue.count}" /></span>
